...
public class Main {
	...
	public static void main(String[] args) throws CloneNotSupportedException {
		...
		Queue<Process> jobQueue = new PriorityQueue<Process>(numberOfTotalProcesses, new SortByArrivalTime());
		...
		Queue<Process> jobQueueInst1 = new PriorityQueue<Process>(numberOfTotalProcesses, new SortByArrivalTime());

		for (Process p : jobQueue) {
			jobQueueInst1.add((Process) p.clone());
		}

		Queue<Process> jobQueueInst2 = new PriorityQueue<Process>(numberOfTotalProcesses, new SortByArrivalTime());

		for (Process p : jobQueue) {
			jobQueueInst2.add((Process) p.clone());
		}

		Queue<Process> jobQueueInst3 = new PriorityQueue<Process>(numberOfTotalProcesses, new SortByArrivalTime());

		for (Process p : jobQueue) {
			jobQueueInst3.add((Process) p.clone());
		}

		Queue<Process> jobQueueInst4 = new PriorityQueue<Process>(numberOfTotalProcesses, new SortByArrivalTime());

		for (Process p : jobQueue) {
			jobQueueInst4.add((Process) p.clone());
		}

		Queue<Process> jobQueueInst5 = new PriorityQueue<Process>(numberOfTotalProcesses, new SortByArrivalTime());

		for (Process p : jobQueue) {
			jobQueueInst5.add((Process) p.clone());
		}

		Queue<Process> jobQueueInst6 = new PriorityQueue<Process>(numberOfTotalProcesses, new SortByArrivalTime());
		...
		RoundRobin RR = new RoundRobin(jobQueueInst5,
				new PriorityQueue<Process>(numberOfTotalProcesses, new SortByArrivalTime()), new LinkedList<Process>(),
				degreeOfMP);

		ExponentialAveraging expAveraging = new ExponentialAveraging(jobQueueInst6,
				new PriorityQueue<Process>(numberOfTotalProcesses, new SortByExponentialAveraging()),
