...
public class ShortestRemainingTimeFirst {
	...
	public ShortestRemainingTimeFirst(Queue<Process> jobQueue, Queue<Process> readyQueue,
			LinkedList<Process> deviceList, int degreeOfMP) {
		super();
	...
	public void applySRJF() {
		...
		while (true) {
			...
			if (!readyQueue.isEmpty() && runningState != null
					&& readyQueue.peek().getTimeOfCurrentCPUBurst() < runningState.getTimeOfCurrentCPUBurst()) {
				System.out.println("some changes will happened :D :D:D :D:D:D:D:DD llil rawwr");
				prevRunningState = runningState;
				displayInfo = true;
